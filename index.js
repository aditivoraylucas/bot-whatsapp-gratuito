...
// ─── GOOGLE SHEETS — getDoc() centralizado ────────────────────────────────────
// BUG 7 FIX: getDoc() é chamado UMA ÚNICA VEZ por operação.
// As funções getSheetSaldo/getSheetHistorico aceitam 'doc' como parâmetro opcional.
// Quando chamadas internamente recebem o doc já carregado; quando chamadas
// isoladamente (retrocompatibilidade) carregam o doc internamente.
...
// Aceita doc opcional — evita segundo getDoc() quando já temos a instância
...
async function getSheetHistorico(doc) {
  const d = doc || await getDoc();
  if (d.sheetCount < 2) return await d.addSheet({ title: 'Historico', headerValues: ['Data','Cliente','Tipo','Produto','Quantidade','Valor'] });
...
async function registrarOuAcumular(clienteEnviado, produto, quantidade) {
  try {
    // BUG 7 FIX: doc carregado uma vez, passado para ambas as funções
...
async function cancelarLancamento(jid, nomeDigitado) {
  try {
    ...
    const lanc = hist[idx];
    hist.splice(idx, 1);
    // BUG 7 FIX: doc carregado uma vez
    const doc = await getDoc();
    const sheet = await getSheetSaldo(doc);
    const rows = await sheet.getRows();
    const sheetH = await getSheetHistorico(doc);
...
async function carregarHistoricoAgrupado(filtroMes) {
  // BUG 7 FIX: doc carregado uma vez, usado para ambas as abas
  const doc = await getDoc();
...
async function gerarResumoDiario() {
  try {
    ...
    // BUG 7 FIX: reutiliza o doc já carregado em carregarHistoricoAgrupado via getSheetSaldo
