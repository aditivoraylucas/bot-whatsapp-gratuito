...
function formatarRespostaCompra(res, produto) {
  ...
  const ehPrimeira = res.qtdNova === res.qtdAcumulada;
...
function normalizarFrasePagamento(linha) {
  ...
  const substituicoes = [
    [/\b(acabou|acaba|acabei)\s+de\s+pagar\b/g,                    'pagou'],
    [/\bja\s+(pagou|pago)\b/g,                                      'pagou'],
    [/\b(quitou|quitei|quitamos|quitaram)\b/g,                      'pagou'],
    [/\b(liquidou|liquidei)\b/g,                                    'pagou'],
    [/\b(efetuou|fez|realizou)\s+(o\s+)?pagamento\b/g,              'pagou'],
    [/\b(me|nos)\s+pagou\b/g,                                       'pagou'],
  ];
...
function parsearPagamento(linha) {
  ...
  const palavrasProc = linhaProcessada.split(/\s+/).filter(Boolean);
  let kwIdx = -1;
...
async function processarTexto(texto, jid, sock) {
  ...
  if (confirmacao) {
    if (Date.now() > confirmacao.expira) {
      ...
      pushLancamento(jid, { tipo: 'compra', cliente: res.cliente, produto: confirmacao.produto, quantidade: qtd, valor: res.valorNovo });
      return formatarRespostaCompra(res, confirmacao.produto);
    }
  }
  ...
  if (ZERAR_PENDENTE[jid]) {
    ...
    if (p0 === 'sim' || p0 === 's') return await zerarCliente(cliente);
  ...
  if (compra) {
    ...
    for (const item of compra.itens) {
      ...
      pushLancamento(jid, { tipo: 'compra', cliente: res.cliente, produto: item.produto, quantidade: item.quantidade, valor: res.valorNovo });
      msgs.push(formatarRespostaCompra(res, item.produto));
    }
    return msgs.join('\n');
  }
